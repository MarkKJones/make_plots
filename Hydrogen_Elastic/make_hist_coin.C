...
void make_hist_coin(TString basename="",Int_t nrun=3288,Double_t pfac=0.981){
   if (basename=="") {
...
// Define branches
 ...
 Double_t  p_etottracknorm;
   tsimc->SetBranchAddress("P.cal.etottracknorm",&p_etottracknorm);
 Double_t  aerosum;
 ...
 Double_t  ThScat;
   ...
   // Define histograms
    TH1F *hW = new TH1F("hW",Form("Run %d ; W (GeV);Counts",nrun), 100, 0.8,5.0);
    HList.Add(hW);
   ...
   TH1F *hPmissz = new TH1F("hPmissz",Form("Run %d ; Pmissz (GeV);Counts",nrun), 100, -.2,.2);
    HList.Add(hPmissz);
     TH1F *hetottracknorm = new TH1F("hetot",Form("Run %d ; Etottrack norm;Counts",nrun), 120, 0.0,2.0);
...
Long64_t nentries = tsimc->GetEntries();
	for (int i = 0; i < nentries; i++) {
		...
		hWcalc->Fill(W_calc);*/
		  if (aerosum>2.) hetottracknorm->Fill(p_etottracknorm);
