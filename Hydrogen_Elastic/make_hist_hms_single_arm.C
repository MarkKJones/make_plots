...
void make_hist_hms_single_arm(TString basename="",Int_t nrun=3288,Double_t pfac=1.0){
   if (basename=="") {
...
// Define branches
 ...
 Double_t  ThScat;
  ...
  // loop over entries
    Double_t th_cent=15.0;
  Double_t Mp = .93827;
   Double_t Ei=2.221;
   Double_t p_spec=2.055;
...
Long64_t nentries = tsimc->GetEntries();
	for (int i = 0; i < nentries; i++) {
      		tsimc->GetEntry(i);
                ...
                hxbeam->Fill(beamx);
                hybeam->Fill(beamy);
		if (gindex>-1  && TMath::Abs(e_delta) < 10.) { 
                hxtar->Fill(e_xtar);
