...
Long64_t data_entries = tsimc->GetEntries();
 ...
 Double_t Scal_BCM1_charge_corr=0;
  ...
  Int_t nscal_reads_2=0;
	for (int i = 0; i < data_entries; i++) {
		...
		if (hgoodscinhit==1 && hgoodstarttime==1) h_hStartTime->Fill(hstarttime);
		 ...
		 if (Hetotnorm > 0.6&&Hhgcer_npeSum>1&& hstarttime!=-1000 && EDTM_timeraw==0) {
		   ...
		   if (hntrack>0 && hdelta < 25 &&  hdelta >-10) {
	...
	Double_t clt = (good_ev2-nEDTM)/(tot_scal_cut_TRIG2-tot_scal_cut_EDTM);
	...
	Double_t good_evTrack_err = TMath::Sqrt(h_goodevTrack->Integral())/clt/calc_treff;
 	//
        cout << " data " << endl;
	cout << nrun << " "<< charge_sum_cut/tot_scal_cut_time  << " "<< charge_sum_cut_corr/tot_scal_cut_time << " " << clt<< " " << lt  << " " << calc_treff << " " << good_ev2/tot_scal_cut_time << " " << good_ev/charge_sum_cut << " " <<  good_ev_err/charge_sum_cut << " " << good_evTrack/charge_sum_cut << " " <<  good_evTrack_err/charge_sum_cut << endl;
	//
