...
void plot_yield(Int_t ntar=3, TString spec="HMS") {
gStyle->SetPalette(1,0);
 ...
 gStyle->SetPadLeftMargin(0.16);
   TString outputpdf;
   outputpdf= "plots/fall18_"+spec+"_24_lumi.pdf";
   Bool_t DumSub= kTRUE;
   //
   //const Int_t ntar=3;
   TString DataFile[ntar];
   TString TarLabel[ntar];
   TString TarNames[4]={"LD2","Carbon","Dummy","LH2"};
   for (Int_t i=0;i<ntar;i++) {
     TarLabel[i]=spec+" "+TarNames[i];
      DataFile[i] = "FALL18_LUMI/"+spec+"_"+TarNames[i]+"_24_corr_data.dat";
   }
   //
