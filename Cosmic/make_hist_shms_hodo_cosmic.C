...
void make_hist_shms_hodo_cosmic(TString basename="",Int_t nrun=2043){
   if (basename=="") {
     cout << " Input the basename of the root file (assumed to be in worksim)" << endl;
     cin >> basename;
   }
   static const char* spec="P";
gStyle->SetPalette(1,0);
 ...
 gStyle->SetPadLeftMargin(0.12);
   ...
   TString outputhist;
   outputhist= "hist/"+basename+"_hodo_cosmic_hist.root";
 TObjArray HList(0);
     TString outputpdf;
...
// Define branches
 ...
 Double_t negtime[plnum][21];
 Double_t postime[plnum][21];
 Double_t negtime_uncorr[plnum][21];
 Double_t postime_uncorr[plnum][21];
 Double_t negtime_corr[plnum][21];
 Double_t postime_corr[plnum][21];
 Double_t negamp[plnum][21];
 Double_t posamp[plnum][21];
 Int_t plcross[plnum]={1,0,3,2};
 Double_t space[plnum]={7.5,-7.5,9.5,-5.0};
 Double_t pos_pad1[plnum]={-45.,45.,-61.75,50.};
 ...
 TH1F* negadchist[plnum][21];
 TH1F* posadchist[plnum][21];
 TH1F* diffhist[plnum][21];
 TH1F* diffadchist[plnum][21];
 TH1F* diffhistcent[plnum][21];
 TH1F* difftdcuncorrcent[plnum][21];
 TH1F* difftdccorrcent[plnum][21];
 TH1F* diffadchistcent[plnum][21];
 TH1F* diffposhistcent[plnum][21];
 TH1F* diffneghistcent[plnum][21];
 TH1F* hdiff_hit2_posdiff[plnum][21];
 TH1F* hdiff_hit2_negdiff[plnum][21];
 for (Int_t ip=0;ip<plnum;ip++) {
 ...
 for (Int_t ipd=0;ipd<npad[ip];ipd++) {
   ...
   diffhistcent[ip][ipd] = new TH1F(Form("histcent_%s_diff_pad_%d",plname[ip],ipd+1),Form("Plane %s Pad %d Center cross pad ; TW Corr Pos-Neg Time ns; COunts",plname[ip],ipd+1),60,-15,15);
   difftdcuncorrcent[ip][ipd] = new TH1F(Form("histcent_%s_tdcuncorrdiff_pad_%d",plname[ip],ipd+1),Form("Plane %s Pad %d Center cross pad ; Uncorr Pos-Neg Time ns; COunts",plname[ip],ipd+1),60,-15,15);
   difftdccorrcent[ip][ipd] = new TH1F(Form("histcent_%s_tdccorrdiff_pad_%d",plname[ip],ipd+1),Form("Plane %s Pad %d Center cross pad ; Corr Pos-Neg Time ns; COunts",plname[ip],ipd+1),60,-15,15);
   ...
   diffposhistcent[ip][ipd] = new TH1F(Form("histcent_%s_diffpos_pad_%d",plname[ip],ipd+1),Form("Plane %s Pad %d Center cross pad ; Pos- Pos(cross pad) ns; COunts",plname[ip],ipd+1),100,-30,30);
   diffneghistcent[ip][ipd] = new TH1F(Form("histcent_%s_diffneg_pad_%d",plname[ip],ipd+1),Form("Plane %s Pad %d Center cross pad ; Neg- Pos(cross pad) ns; COunts",plname[ip],ipd+1),100,-30,30);
   hdiff_hit2_posdiff[ip][ipd] = new TH1F(Form("hdiff_%s_diffpos_2hit_pad_%d",plname[ip],ipd+1),Form("Plane %s  ; Time diff pos pad %d - pad %d ns; COunts",plname[ip],ipd+1,ipd+2),60,-15,15);
   hdiff_hit2_negdiff[ip][ipd] = new TH1F(Form("hdiff_%s_diffneg_2hit_pad_%d",plname[ip],ipd+1),Form("Plane %s  ; Time diff neg pad %d - pad %d ns; COunts",plname[ip],ipd+1,ipd+2),60,-15,15);
...
// loop over entries
 ...
 Double_t dcut[4]={2.5,2.5,10.,2.5};
...
if (plnhits[0]==1&&plnhits[1]==1&&plnhits[2]==1&&plnhits[3]==1) {
  //  if (TMath::Abs(xfp) < 40) {
    ...
    hypfp->Fill(ypfp);
		...
		//}		  
 for (Int_t ip=0;ip<plnum;ip++) {
   if (TMath::Abs(xfp) < 40 && xpfp <0) hfptime[ip]->Fill(fptime[ip]);
 for (Int_t ipd=0;ipd<npad[ip];ipd++) {
  ...
  for (Int_t ip=0;ip<plnum;ip++) {
   for (Int_t ipd=0;ipd<npad[ip];ipd++) {
    ...
    if (postime[ip][ipd] < 3000 && negtime[ip][ipd] < 3000) {
	    ...
	    diffadchist[ip][ipd]->Fill(posamp[ip][ipd]-negamp[ip][ipd]);
