...
void plot_hist_hodo_cosmic(TString basename,Int_t plot_pm_diff=-1,Int_t plot_plane_diff=-1,Int_t plot_2hit_diff=-1,Int_t plot_adc_diff=1) {
 gROOT->Reset();
 gStyle->SetOptStat(0);
 gStyle->SetOptFit(11);
 ...
 gStyle->SetPadLeftMargin(0.14);
     TString outputpdf;
 outputpdf="plots/"+basename+"_hms_hodo_adc.pdf";
  TString inputroot;
 static const Int_t nftot=2;
   TFile *fhistroot;
     inputroot="hist/"+basename+"_hms_hodo_cosmic_hist.root";
 ...
 TH1F* diffhistcent[plnum][21];
 TH1F* difftdccorrcent[plnum][21];
 TH1F* diffadchistcent[plnum][21];
 TH1F* negadchistcent[plnum][21];
 TH1F* posadchistcent[plnum][21];
 TH1F* diffposhistcent[plnum][21];
 TH1F* diffneghistcent[plnum][21];
 TH1F* diffpos2hitcent[plnum][21];
 TH1F* diffneg2hitcent[plnum][21];
  for (Int_t ip=0;ip<plnum;ip++) {
    for (Int_t ipd=0;ipd<npad[ip];ipd++) {
      TString hname = Form("histcent_%s_diff_pad_%d",plname[ip],ipd+1);
   diffhistcent[ip][ipd] = (TH1F*)fhistroot->Get(hname);
      hname = Form("histcent_%s_tdccorrdiff_pad_%d",plname[ip],ipd+1);
   difftdccorrcent[ip][ipd] = (TH1F*)fhistroot->Get(hname);
      hname = Form("hist_%s_adcneg_pad_%d",plname[ip],ipd+1);
   negadchistcent[ip][ipd] = (TH1F*)fhistroot->Get(hname);
      hname = Form("hist_%s_adcpos_pad_%d",plname[ip],ipd+1);
   posadchistcent[ip][ipd] = (TH1F*)fhistroot->Get(hname);
      hname = Form("histcent_%s_adcdiff_pad_%d",plname[ip],ipd+1);
   diffadchistcent[ip][ipd] = (TH1F*)fhistroot->Get(hname);
      hname = Form("histcent_%s_diffpos_pad_%d",plname[ip],ipd+1);
   diffposhistcent[ip][ipd] =(TH1F*)fhistroot->Get(hname);
      hname = Form("hdiff_%s_diffpos_2hit_pad_%d",plname[ip],ipd+1);
   diffpos2hitcent[ip][ipd] =(TH1F*)fhistroot->Get(hname);
       hname = Form("hdiff_%s_diffneg_2hit_pad_%d",plname[ip],ipd+1);
   diffneg2hitcent[ip][ipd] =(TH1F*)fhistroot->Get(hname);
      hname = Form("histcent_%s_diffneg_pad_%d",plname[ip],ipd+1);
   diffneghistcent[ip][ipd] = (TH1F*)fhistroot->Get(hname);
    }
  } 
  //
...
//
 if (plot_adc_diff==1) {
     ...
     for (Int_t nh=0;nh<plnum;nh++) {
	  ...
	  cplotdiffpad[nh]->cd(cnt);
          diffadchistcent[nh][np]->DrawNormalized();
	  //	  gPad->SetLogy();
          cnt++;
        }      
    ...
    TCanvas *cplotpospad[plnum];
     for (Int_t nh=0;nh<plnum;nh++) {
	  ...
	  cplotpospad[nh]->cd(cnt);
          posadchistcent[nh][np]->DrawNormalized();
          negadchistcent[nh][np]->SetLineColor(2);
          negadchistcent[nh][np]->DrawNormalized("same");
	  //	  gPad->SetLogy();
          cnt++;
        }      
 ...
 if (plot_2hit_diff==1) {
  //
      Double_t pos_2hit_mean[4][21];
     Double_t pos_2hit_padnum[4][21];
  outputpdf="plots/"+basename+"_shms_hodo_diff_2hit_pos.pdf";
    TCanvas *cplot2hit[plnum];
     for (Int_t nh=0;nh<plnum;nh++) {
	  ...
	  cplot2hit[nh]->cd(cnt);
          diffpos2hitcent[nh][np]->DrawNormalized();
	  pos_2hit_mean[nh][cnt-1]=diffpos2hitcent[nh][np]->GetMean();
	  pos_2hit_padnum[nh][cnt-1]=np+1;
	  //	  gPad->SetLogy();
          cnt++;
        }      
  ...
  outputpdf="plots/"+basename+"_shms_hodo_diff_2hit_pos_mean.pdf";
     ...
     for (Int_t nh=0;nh<plnum;nh++) {
       ...
       gr_pos_2hit_mean[nh] = new TGraph(totp,pos_2hit_padnum[nh],pos_2hit_mean[nh]);
       gr_pos_2hit_mean[nh]->SetMarkerColor(1);
       gr_pos_2hit_mean[nh]->SetMarkerStyle(22);
        mgr_pos_2hit[nh]->Add(gr_pos_2hit_mean[nh]);
 ...
 //
  //
      Double_t neg_2hit_mean[4][21];
     Double_t neg_2hit_padnum[4][21];
    ...
    TCanvas *cplot2hitneg[plnum];
     for (Int_t nh=0;nh<plnum;nh++) {
	  ...
	  cplot2hitneg[nh]->cd(cnt);
          diffneg2hitcent[nh][np]->DrawNormalized();
	  neg_2hit_mean[nh][cnt-1]=diffneg2hitcent[nh][np]->GetMean();
	  neg_2hit_padnum[nh][cnt-1]=np+1;
	  //	  gPad->SetLogy();
          cnt++;
        }      
  ...
  outputpdf="plots/"+basename+"_shms_hodo_diff_2hit_neg_mean.pdf";
     ...
     for (Int_t nh=0;nh<plnum;nh++) {
       ...
       Int_t totp = npad_hi[nh]-npad_lo[nh]-1;
       gr_neg_2hit_mean[nh] = new TGraph(totp,neg_2hit_padnum[nh],neg_2hit_mean[nh]);
       gr_neg_2hit_mean[nh]->SetMarkerColor(1);
       gr_neg_2hit_mean[nh]->SetMarkerStyle(22);
      mgr_neg_2hit[nh]->Add(gr_neg_2hit_mean[nh]);
  ...
  outputpdf="plots/"+basename+"_shms_hodo_diff_pos.pdf";
     Double_t pl_pos_mean[4][21];
     Double_t pl_pos_padnum[4][21];
    ...
    for (Int_t nh=0;nh<plnum;nh++) {
	  ...
	  cplotposdiff[nh]->cd(cnt);
          diffposhistcent[nh][np]->DrawNormalized();
	  pl_pos_padnum[nh][cnt-1]=np+1;
	  pl_pos_mean[nh][cnt-1]=diffposhistcent[nh][np]->GetMean();
	  //	  gPad->SetLogy();
          cnt++;
        }      
  ...
  outputpdf="plots/"+basename+"_shms_hodo_diff_neg.pdf";
     Double_t pl_neg_mean[4][21];
     Double_t pl_neg_padnum[4][21];
     ...
     for (Int_t nh=0;nh<plnum;nh++) {
	  ...
	  cplotnegdiff[nh]->cd(cnt);
          diffneghistcent[nh][np]->DrawNormalized();
	  pl_neg_padnum[nh][cnt-1]=np+1;
	  pl_neg_mean[nh][cnt-1]=diffneghistcent[nh][np]->GetMean();
	  //	  gPad->SetLogy();
          cnt++;
        }      
	...
	if (nh==3)   cplotnegdiff[nh]->Print(outputpdf+")");  
     ...
     for (Int_t nh=0;nh<plnum;nh++) {
       ...
       Int_t totp = npad_hi[nh]-npad_lo[nh];
       gr_pl_pos_mean[nh] = new TGraph(totp,pl_pos_padnum[nh],pl_pos_mean[nh]);
       gr_pl_pos_mean[nh]->SetMarkerColor(1);
       gr_pl_pos_mean[nh]->SetMarkerStyle(22);
      mgr[nh]->Add(gr_pl_pos_mean[nh]);
    ...
    outputpdf="plots/"+basename+"_shms_hodo_diff_pl_neg_mean.pdf";
     ...
     for (Int_t nh=0;nh<plnum;nh++) {
       ...
       Int_t totp = npad_hi[nh]-npad_lo[nh];
       gr_pl_neg_mean[nh] = new TGraph(totp,pl_neg_padnum[nh],pl_neg_mean[nh]);
       gr_pl_neg_mean[nh]->SetMarkerColor(1);
       gr_pl_neg_mean[nh]->SetMarkerStyle(22);
      mgr_pl_neg[nh]->Add(gr_pl_neg_mean[nh]);
  ...
  //
     ...
     for (Int_t nh=0;nh<plnum;nh++) {
	  ...
	  cplotcomppmdiff[nh]->cd(cnt);
          diffhistcent[nh][np]->DrawNormalized();
          difftdccorrcent[nh][np]->SetLineColor(2);
          difftdccorrcent[nh][np]->DrawNormalized("same");
	  //	  gPad->SetLogy();
          cnt++;
        }      
  ...
  //
     Double_t pm_mean[4][21];
     Double_t pm_mean_out[4][21];
     Double_t pm_padnum[4][21];
     for (Int_t nh=0;nh<plnum;nh++) {
 ...
 outputpdf="plots/"+basename+"_shms_hodo_diff_pm.pdf";
     ...
     for (Int_t nh=0;nh<plnum;nh++) {
	  ...
	  cplotpmdiff[nh]->cd(cnt);
          diffhistcent[nh][np]->DrawNormalized();
	  pm_padnum[nh][cnt-1]=np+1;
	  pm_mean[nh][cnt-1]=diffhistcent[nh][np]->GetMean();
	  pm_mean_out[nh][np]=diffhistcent[nh][np]->GetMean();
	  //	  gPad->SetLogy();
          cnt++;
        }      
  ...
  //
     ...
     for (Int_t nh=0;nh<plnum;nh++) {
       ...
       cplotpmmean->cd(nh+1);
       Int_t totp = npad_hi[nh]-npad_lo[nh];
       gr_pm_mean[nh] = new TGraph(totp-1,pm_padnum[nh],pm_mean[nh]);
       gr_pm_mean[nh]->SetMarkerColor(1);
       gr_pm_mean[nh]->SetMarkerStyle(22);
      mgr[nh]->Add(gr_pm_mean[nh]);
