...
void comp_hist_shms_hodo(TString basename, TString basename1) {
 gROOT->Reset();
 gStyle->SetOptStat(0);
 gStyle->SetOptFit(11);
 ...
 gStyle->SetPadLeftMargin(0.14);
     TString outputpdf;
 outputpdf="plots/"+basename+"_shms_hodo.pdf";
  TString inputroot;
...
Int_t npad_hi[plnum]={13,13,14,18};
 ...
 TH1D *hn2[4][21];
    TCanvas *cplotnegpad[plnum];
     for (Int_t nh=0;nh<plnum;nh++) {
       ...
       if (nh!=plnum-1) cplotnegpad[nh]->Divide(5,3);
       if (nh==plnum-1) cplotnegpad[nh]->Divide(5,4);
	...
	if (nh==0)   cplotnegpad[nh]->Print(outputpdf+"(");  
	if (nh!=0)   cplotnegpad[nh]->Print(outputpdf);  
     }
  //
 TH1D *hp1[4][21];
 TH1D *hp2[4][21];
	...
	if (nh==plnum-1)   cplotpospad[nh]->Print(outputpdf+")");  
	if (nh!=plnum-1)   cplotpospad[nh]->Print(outputpdf);  
     }
  //
