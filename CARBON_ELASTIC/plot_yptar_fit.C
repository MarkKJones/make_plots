...
void plot_yptar_fit(TString basename=""){
  //
   if (basename=="") {
  ...
  TFile *fhistroot;
   fhistroot =  new TFile(inputroot);
    ...
    for (int nny=0;nny<11;nny++) {
      hyptardiffnew[nd][nnx][nny] = (TH1F*)fhistroot->Get(Form("hyptardiffnew_%d_%d_%d",nd,nnx,nny));
    if (!hyptardiffnew[nd][nnx][nny] ) cout << " no hist = " << Form("hyptardiffnew_%d_%d_%d",nd,nnx,nny) << endl;
      hyptardiffold[nd][nnx][nny] = (TH1F*)fhistroot->Get(Form("hyptardiffold_%d_%d_%d",nd,nnx,nny));
    }
    }
    }
    //
    if (1==-1) {
    TCanvas *cind = new TCanvas("cind"," individual", 900,700);
      cind->Divide(1,1);
      cind->cd(1);
    for (int nd=0;nd<18;nd++) {
    for (int nnx=0;nnx<11;nnx++) {
    for (int nny=0;nny<11;nny++) {
      hyptardiffnew[nd][nnx][nny]->Draw();
      hyptardiffnew[nd][nnx][nny]->SetLineColor(2);
      hyptardiffold[nd][nnx][nny]->Draw("same");
      gPad->WaitPrimitive();
      cout << nd << " " << nnx << " " << nny << endl;
    }}}   
    }else {
    //
   ...
   Double_t x[18][11][11];
    ...
    for (int nnx=0;nnx<11;nnx++) {
    for (int nny=0;nny<11;nny++) {
      if (hyptardiffnew[nd][nnx][nny]->Integral()>50) {
      meannew[nd][nnx][nny]=hyptardiffnew[nd][nnx][nny]->GetMean();
      rmsnew[nd][nnx][nny]=hyptardiffnew[nd][nnx][nny]->GetRMS();
      meanold[nd][nnx][nny]=hyptardiffold[nd][nnx][nny]->GetMean();
      rmsold[nd][nnx][nny]=hyptardiffold[nd][nnx][nny]->GetRMS();
      xmeannew[nd][nny][nnx]=hyptardiffnew[nd][nnx][nny]->GetMean();
      xrmsnew[nd][nny][nnx]=hyptardiffnew[nd][nnx][nny]->GetRMS();
      xmeanold[nd][nny][nnx]=hyptardiffold[nd][nnx][nny]->GetMean();
      xrmsold[nd][nny][nnx]=hyptardiffold[nd][nnx][nny]->GetRMS();
      } else {
    ...
    //
   pdffile="plots/"+basename+"_yptardiff_nx.pdf";
      TCanvas *cplot[18];
    ...
    for (int nnx=0;nnx<11;nnx++) {
     ...
     mgr[nd][nnx]->SetTitle(Form("Delta run= %d, Sieve Nx = %d; Ny sieve; Yptar diff",-12+2*nd,nnx));
     mgr[nd][nnx]->Draw("AP");
    }
   ...
   //
   pdffile="plots/"+basename+"_yptardiff_ny.pdf";
    //
      TCanvas *cplot2[18];
    ...
    for (int nny=0;nny<11;nny++) {
     ...
     mgrx[nd][nny]->SetTitle(Form("Delta run = %d, Sieve Ny = %d; Nx sieve; Yptar diff",-12+2*nd,nny));
     mgrx[nd][nny]->Draw("AP");
    }
   ...
   //
   pdffile="plots/"+basename+"_yptardiff.pdf";
      Double_t max_ny;
      TCanvas *cplot3[18];
    for (int nd=0;nd<18;nd++) {
    ...
    for (int nny=0;nny<11;nny++) {
      ...
      max_ny=0;
      for (int nnx=0;nnx<11;nnx++) {
	if (hyptardiffnew[nd][nnx][nny]->GetMaximum()>max_ny) max_ny = hyptardiffnew[nd][nnx][nny]->GetMaximum(); 
	hyptardiffnew[nd][nnx][nny]->SetTitle(Form("Delta run =%d Ny = %d ",-12+2*nd,nny));
      }
      //gPad->SetLogy();
      // cout << nd << " " << nny << " " << max_ny << " " << endl;
      for (int nnx=0;nnx<11;nnx++) {
	if ( hyptardiffnew[nd][nnx][nny]->Integral()>0) {
      hyptardiffnew[nd][nnx][nny]->SetMaximum(max_ny*1.1); 
      hyptardiffnew[nd][nnx][nny]->SetMinimum(0); 
      hyptardiffnew[nd][nnx][nny]->SetAxisRange(-.5,.5,"X"); 
      hyptardiffnew[nd][nnx][nny]->Draw("same"); 
      hyptardiffnew[nd][nnx][nny]->SetLineColor(nnx); 
	}
      }
    }
