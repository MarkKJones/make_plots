...
void make_hist_lumi_hms2_jpsi(TString basename="",Int_t nrun=2043,Double_t threshold_cut=2.0){
   if (basename=="") {
...
Long64_t data_entries = tsimc->GetEntries();
	for (int i = 0; i < scal_entries; i++) {
	  ...
	  if (TMath::Abs(Scal_BCM4B_current-threshold_cut) < 5.) {
