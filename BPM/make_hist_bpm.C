...
void make_hist_bpm(TString basename="",Int_t nrun=2043){
   if (basename=="") {
...
Long64_t nentries = tsimc->GetEntries();
	for (int i = 0; i < nentries; i++) {
		  ...
		  hSCXraw->Fill(SCXraw);
		  hSCYraw->Fill(SCYraw);
		  SAXpos=-1*(SAX_off+SAX_scale*SAXraw);
		  SAYpos=SAY_off+SAY_scale*SAYraw;
		  SBXpos=-1*(SBX_off+SBX_scale*SBXraw);
		  SBYpos=SBY_off+SBY_scale*SBYraw;
		  SCXpos=-1*(SCX_off+SCX_scale*SCXraw);
