...
void comp_hist_hms_hodo_raw(TString basename, TString basename1) {
 gROOT->Reset();
 gStyle->SetOptStat(0);
 gStyle->SetOptFit(11);
 ...
 static const Int_t nftot=2;
   TFile *fhistroot[nftot];
     inputroot="hist/"+basename+"_hms_hodo_raw_hist.root";
     cout << " infile root = " << inputroot << endl;
   fhistroot[0] =  new TFile(inputroot);
     inputroot="hist/"+basename1+"_hms_hodo_raw_hist.root";
     cout << " infile root = " << inputroot << endl;
 ...
 //     
  ...
  for (Int_t ic=0;ic<3;ic++) {
   ...
   for (Int_t is=0;is<ns;is++) {
     ...
     if (ip==0 || ip==2) can[ip][is][ic]->Divide(2,8);
     if (ip==1 || ip==3) can[ip][is][ic]->Divide(2,5);
        for (Int_t ipad=0;ipad<npad[ip];ipad++) {
	  can[ip][is][ic]->cd(ipad+1);
	  if (ic==0) {
	    h_adcpulsetimediff[ip][is][ipad][0]->Draw();	    
	    h_adcpulsetimediff[ip][is][ipad][1]->Draw("same");
	    h_adcpulsetimediff[ip][is][ipad][1]->SetLineColor(2);
	  }
	  if (ic==1) {
	    h_adcpulsetime[ip][is][ipad][0]->Draw();	    
	    h_adcpulsetime[ip][is][ipad][1]->Draw("same");
	    h_adcpulsetime[ip][is][ipad][1]->SetLineColor(2);
	  }
	  if (ic==2) {
	    h_tdctime[ip][is][ipad][0]->Draw();	    
	    h_tdctime[ip][is][ipad][1]->Draw("same");
	    h_tdctime[ip][is][ipad][1]->SetLineColor(2);
	  }
        }
    outputpdf=Form("plots/comp_hms_hodo_raw_%s_%s_%s.pdf",plname[ip],sname[is],tname[ic]);
