...
void make_hist_hms_hodo_raw(TString basename="",Int_t nrun=2043){
   if (basename=="") {
...
gStyle->SetPalette(1,0);
 ...
 gStyle->SetPadLeftMargin(0.12);
   ...
   TString outputhist;
   outputhist= "hist/"+basename+"_hms_hodo_raw_hist.root";
 TObjArray HList(0);
 //
...
//
 ...
 for (Int_t is=0;is<ns;is++) {
   tsimc->SetBranchAddress(Form("Ndata.H.hod.%s.%s%s",plname[ip],sname[is],adcname[0]),&adc_nhits[ip][is]) ;   
   tsimc->SetBranchAddress(Form("Ndata.H.hod.%s.%s%s",plname[ip],sname[is],tdcname[0]),&tdc_nhits[ip][is]) ;   
 for (Int_t it=0;it<adcsigtypes;it++) {
   tsimc->SetBranchAddress(Form("H.hod.%s.%s%s",plname[ip],sname[is],adcname[it]),&adc[ip][is][it]) ;   
 }
 for (Int_t it=0;it<tdcsigtypes;it++) {
   tsimc->SetBranchAddress(Form("H.hod.%s.%s%s",plname[ip],sname[is],tdcname[it]),&tdc[ip][is][it]) ;   
 }
 }}
 Double_t starttime;
...
TH1F* h_tdctime[plnum][ns][16];
 ...
 for (Int_t ipad=0;ipad<npad[ip];ipad++) {
   ...
   h_tdctime[ip][is][ipad] = new TH1F(Form("h_tdctime_%d_%d_%d",ip,is,ipad),Form("; %s %s Pad %d TDC time",plname[ip],sname[is],ipad+1),200.,-0.,100.);   
 ...
 Long64_t nentries = tsimc->GetEntries();
	for (int i = 0; i < nentries; i++) {
		...
		for (Int_t nh=0;nh<tdc_nhits[ip][is];nh++) {
		  ...
		  tdcpad[ip][is][np]=(tdc[ip][is][1][nh]+1580)*0.09776;
		}
		for (Int_t np=0;np<16;np++) {
