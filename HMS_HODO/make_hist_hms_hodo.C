...
//
 ...
 for (Int_t ip=0;ip<plnum;ip++) {
   tsimc->SetBranchAddress(Form("Ndata.H.hod.%s.Clus.Pos",plname[ip]),&ncl[ip]) ;
   tsimc->SetBranchAddress(Form("H.hod.%s.Clus.Pos",plname[ip]),&pos[ip]) ;
   tsimc->SetBranchAddress(Form("H.hod.%s.Clus.Size",plname[ip]),&size[ip]) ;
   tsimc->SetBranchAddress(Form("H.hod.%s.Clus.Flag",plname[ip]),&flag[ip]) ;
   tsimc->SetBranchAddress(Form("H.hod.%s.Clus.UsedFlag",plname[ip]),&usedflag[ip]) ;
 }
 //
 TH1F*  hgoodscinhit = new TH1F("hgoodscinhit",";Good Scin Hit ;",10,0,10);
 TH1F*  hxdiff_nclusone = new TH1F("hxdiff_nclusone","One cluster in plane; X1-X2 (cm)  ;",100,-20,20);
 TH1F*  hxdiff_nclusone_2 = new TH1F("hxdiff_nclusone_2","No shift One cluster in plane; X1-X2 (cm)  ;",100,-20,20);
 TH1F*  hydiff_nclusone = new TH1F("hydiff_nclusone","One cluster in plane; Y1-Y2 (cm)  ;",100,-20,20);
 TH2F*  hxdiff_xpos_nclusone = new TH2F("hxdiff_xpos_nclusone","One cluster in plane; X1-X2 (cm)  ; X1",100,-20,20,100,-60,60);
 TH2F*  hydiff_ypos_nclusone = new TH2F("hydiff_ypos_nclusone","One cluster in plane; Y1-Y2 (cm)  ; Y1",100,-20,20,100,-40,40);
 TH1F*  hclus[plnum];
 TH1F*  hpos[plnum];
