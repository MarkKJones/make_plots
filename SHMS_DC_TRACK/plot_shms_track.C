...
void plot_shms_track(TString basename="",Int_t nrun=2043){
   if (basename=="") {
...
gStyle->SetPalette(1,0);
 ...
 gStyle->SetPadLeftMargin(0.12);
   ...
   TString outputhist;
   outputhist= basename+"_shms_track_hist.root";
 TObjArray HList(0);
     TString outputpdf;
    outputpdf=basename+"_shms_track.pdf";
...
Double_t  edtm_time;
 ...
 Double_t  pstime;
   tsimc->SetBranchAddress("P.hod.starttime",&pstime);
 Double_t  pgoodstime;
   tsimc->SetBranchAddress("P.hod.goodstarttime",&pgoodstime);
   /*
 Double_t  delta;
 ...
 Double_t  ypfp;
  ...
  //
   temp=Form("E/p > 0 Run %d ; Cal  Y ; Cal X ",nrun);
   TH2F *h_calXY = new TH2F("h_calXY",temp,80,-180,180.,80,-180,180.0);
   temp=Form("Npesum = 0 E/p==0 Run %d ; Cal  Y ; Cal X ",nrun);
   TH2F *h_calXY_zero= new TH2F("h_calXY_zero",temp,80,-180,180.,80,-180,180.0);
   TH1F *hstarttime_zero = new TH1F("hstarttime_zero", Form(" Run %d ; Starttime ; Counts",nrun), 240, -2000.0,120.);
   TH1F *hstarttime = new TH1F("hstarttime", Form(" Run %d ; Starttime ; Counts",nrun), 240, 0.0,120.);
   ...
   TH1F *hhgcer_sum = new TH1F("hhgcer_sum", Form(" Run %d ; HG Cer Sum Npe ; Counts",nrun), 200, 0.0,50);
   ...
   TH2F *hxfp_xpfp_zero = new TH2F("hxfp_xpfp_zero", Form("sumnpe==0 E/p==0 Run %d ; X_fp (cm) ; Xp_fp",nrun), 200,-40.,40.,200,-.1,.1);
  ...
  TH2F *hxfp_yfp_proj_pion[4];
  TH2F *hxfp_yfp_proj_elec[4];
  TH2F *hxfp_yfp_proj_zero[4];
	for (int i = 0; i <4; i++) {  
	  hxfp_yfp_proj[i] = new TH2F(Form("hxfp_yfp_proj_%d",i), Form(" HG >0 0< NGnpe < %3.2f , Cal E/p< %3.2f Run %d Zoff = %5.2f; X_fp (cm) ; Y_fp",cer_cut,ep_cut,nrun,zoff[i]), 200,-40.,40.,200,-40.,40.);
	  hxfp_yfp_proj_zero[i] = new TH2F(Form("hxfp_yfp_proj_zero_%d",i), Form("HG ==0 NG==0 , Cal E/p==0 Run %d Zoff = %5.2f; X_fp (cm) ; Y_fp",nrun,zoff[i]), 200,-40.,40.,200,-40.,40.);
	  ...
	  hxfp_yfp_proj_pion[i] = new TH2F(Form("hxfp_yfp_proj_pion_%d",i), Form("NG=0, HG > 0. , Cal E/p< %3.2f Run %d Zoff = %5.2f; X_fp (cm) ; Y_fp",ep_cut,nrun,zoff[i]), 200,-40.,40.,200,-40.,40.);
	}
	/*
...
// nentries=300000;
	...
	*/
	hhgcer_sum->Fill(hgsumnpe);
	hngcer_sum->Fill(sumnpe);
	if (xfp <1000 && xfp >-1000 && pgoodstime==1) {
	  if (pgoodstime==1 && sumnpe==0 && hgsumnpe==0 && etotnorm==0) hstarttime_zero->Fill(pstime);
	  if (pgoodstime==1) hstarttime->Fill(pstime);
		  ...
		  if  (etotnorm==0) {
		    ...
		    htime_0_zero->Fill(trig2_time);
                    if (pstime >0 &&sumnpe==0)  hxfp_xpfp_zero->Fill(xfp,xpfp);
		  ...
		  hetotnorm->Fill(etotnorm);
                  ...
                  if(etracknorm==0 && sumnpe==0 && pstime > 0) {
                    for (int j = 0; j <4; j++) {  
		  ...
		  if (sumnpe==0 && etracknorm==0) h_calXY_zero->Fill(yfp+ypfp*300.,xfp+xpfp*300.);
 if (sumnpe > cer_cut && etracknorm!=0) h_calXY->Fill(yfp+ypfp*300.,xfp+xpfp*300.);
		if (sumnpe !=0 && etracknorm!=0) {
                   if  (sumnpe > cer_cut && etracknorm > ep_cut ) {
                	for (int ii = 0; ii <4; ii++) {
		    ...
		    htime_0_elec->Fill(trig2_time);
		       ...
		       hxfp_yfp_proj_elec[j]->Fill(xfp+xpfp*zoff[j],yfp+ypfp*zoff[j]);
                    ...
                    if ((xfp+xpfp*zoff[3]) > 14) hxfp_yfp_win->Fill(xfp,yfp);
                     ...
                     for (int j = 0; j <4; j++) {  
		       if (hgsumnpe!=0) hxfp_yfp_proj_pion[j]->Fill(xfp+xpfp*zoff[j],yfp+ypfp*zoff[j]);
		       if (hgsumnpe==0) hxfp_yfp_proj[j]->Fill(xfp+xpfp*zoff[j],yfp+ypfp*zoff[j]);
		     }
		  }
		}
	}
...
TCanvas *c1 = new TCanvas("c1", "Etrack norm", 700,700);
    TLegend *l1=  new TLegend(.15,.75,.75,.95,"");;
     l1->AddEntry(hetracknorm," All tracks");
     l1->AddEntry(hetracknorm_win,"Tracks with X>14 at z=-307");
c1->Divide(1,2);
c1->cd(1);
 gPad->SetLogz();
...
c1x->cd(2);
 ...
 for (int j = 0; j <4; j++) {  
   ...
   if (j==3) exitwindow->Draw();
   cr4[j]->cd(4);
   hxfp_yfp_proj_pion[j]->Draw("colz");
   if (j==3) exitwindow->Draw();
   outputpdf="plots/"+basename+"_shms_track.pdf";
...
c3->cd(3);
 hxfp_yfp_elec->Draw("colz");
    c3->Print(outputpdf);
    //
TCanvas *cCal = new TCanvas("cCal", "Xcal v Ycal", 700,700);
cCal->Divide(1,2);
cCal->cd(1);
 h_calXY->Draw("colz");
cCal->cd(2);
 h_calXY_zero->Draw("colz");
    outputpdf="plots/"+basename+"_shms_track.pdf)";
    cCal->Print(outputpdf);
    //
    } // if 1==-1
 //
    if (1==-1) {
