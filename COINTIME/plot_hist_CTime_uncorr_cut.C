...
void plot_hist_CTime_uncorr_cut(TString basename) {
 gROOT->Reset();
 gStyle->SetOptStat(0);
 gStyle->SetOptFit(11);
 ...
 //
     TString outputpdf;
 outputpdf="plots/"+basename+"_CTime_uncorr_cut.pdf";
 TString  inputroot;
   TFile *fhistroot;
...
TH1F *htdc_uncorr[plnum][iside][16];
 ...
 for (Int_t ipad=0;ipad<npad[ipl];ipad++) {
    // cout <<  hname[ip] << endl;
       htdc_uncorr_pm[ipl][ipad] = (TH2F*)fhistroot->Get(Form("tdc_cut_uncorr_pm_%s_pad_%d",plname[ipl],ipad));
       if (!htdc_uncorr_pm[ipl][ipad]) cout << " no 2dhist = " << ipl << " " << ipad << endl;
 for (Int_t is=0;is<iside;is++) {
   htdc_uncorr[ipl][is][ipad] = (TH1F*)fhistroot->Get(Form("tdc_cut_uncorr_%s_%s_pad_%d",plname[ipl],sidename[is],ipad));
