...
void make_hist_CTime_jpsi(TString basename="",Int_t nrun=1267,Double_t cpeak=45){
   if (basename=="") {
...
// Define branches
 ...
 Double_t  pcer;
   tsimc->SetBranchAddress("P.ngcer.npeSum",&pcer);
 Double_t  hcer;
...
// loop over entries
	...
	for (int ie = 0; ie < nentries; ie++) {
		...
		if (hcer > 0.5 && pcer > 2.0 && petottracknorm >0.8 && hetottracknorm >0.8 ) heposCoinTime_ROC1_cut->Fill(eposCoinTime_ROC1);
