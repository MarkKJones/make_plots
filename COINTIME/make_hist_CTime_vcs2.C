...
void make_hist_CTime_vcs2(TString basename="",Int_t nrun=1267,Double_t cpeak=45){
   if (basename=="") {
...
Double_t shmstdc_tofcorr[plnum][iside][21];
 ...
 Double_t  hstarttime;
   ...
   HList.Add(haero_cut);
    ...
    TH2F *hCoinTime_RAW_ROC2_pdelta = new TH2F("hcoinTime_RAW_ROC2_pdelta",Form("Run %d ; CoinTime_RAW_ROC2  ; SHMS delta",nrun),600, -50.,100.0,100,-10,20);
    HList.Add(hCoinTime_RAW_ROC2_pdelta);
...
// loop over entries
	...
	for (int i = 0; i < nentries; i++) {
		...
		if (selcut) {
		    ...
		    htdc_uncorr[ipl][is][ipad]->Fill(tdc_uncorr[ipl][is][ipad]);
		     ...
		     if (shmstdc_uncorr[ipl][0][ipad]<1000&&shmstdc_uncorr[ipl][1][ipad]<1000 ) {
                   ...
                   for (Int_t is=0;is<iside;is++) {
                     if (shmstdc_uncorr[ipl][is][ipad]<1000&&shmstdc_uncorr[ipl][is][ipad]>-500) 		    hshmstdc_uncorr[ipl][is][ipad]->Fill(shmstdc_uncorr[ipl][is][ipad]);
		  }}}
		...
		hCoinTime_RAW_ROC2_pdelta->Fill(CoinTime_RAW_ROC2,delta);
